...
VALUES = ['2','3','4','5','6','7','8','9','X','J','Q','K','A'] # X -> 10
...
def tell_hand(hand):
    ...
    if hand_is_valid != "True":
        ...
        if "".join(ordered_values) == "XJQKA":
            ...
            return "High Card", order_by_rank(hand)[-1] #return highest card
...
def make_set_counts(h):
    qtys_with_values = make_value_counts(h).most_common()
    qtys_only = [x[1] for x in qtys_with_values]
    return qtys_only

def test():
    RF = tell_hand(['AH', 'QH', 'KH', 'XH', 'JH'])
    print "Testing Royal Flush: %r, %r" % (RF, RF == "Royal Flush")
    SF = tell_hand(['KD', 'QD', 'JD', 'XD', '9D'])
