...
def solve_for_i():
	volt = "s"
	while type(volt) != int:
		volt = input("What is the voltage, in volts, of the circuit? \n>")
		if type(volt) == int or type(volt) == float:
			resist = input("Great. What is the resistance, in ohms, of the circuit? \n>")
			if type(resist) == int or type(resist) == float:
				current = volt / resist
				print "In accorance with Ohm's Law, the current on this circuit must be: %r amperes." % current
				break
			else:
