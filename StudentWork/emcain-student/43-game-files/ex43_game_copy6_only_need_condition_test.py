...
class Player(object):
	...
	#stuff that should happen after moving:
		...
		print "You see a", self.the_boat.get_person()
		print "What do you want to do?"
		print "1. talk to the", self.the_boat.get_person()  
...
class Npc(object):
	...
	def interact(self):
		...
		if self.move_condition_needed.fulfilled == True: #evaluates to true/satisfied--for some reason this is happening each time. i think the problem is that people's "move condition needed" is always yes???
...
class Map(object):
	...
	def __init__(self):
		...
		baker = Npc("baker wearing an apron", "She has just baked a tray of cookies. \nShe says, 'Here, have a cookie!'", "She says, 'Enjoy your cookie!'", self.condits["HasCookie"], self.condits["StartsNo"])
		self.people[3] = baker 
		...
		midage_woman = Npc("middle-aged woman", "She says, 'Can you help me clean this mess?' \nYou spend a few minutes helping her sort through her family's belongings. \nYou find a baby toy. 'Go ahead and take that,' she says. 'My kids are much too old for it now.'", "She says, 'Thanks so much for helping me clean!'", self.condits["BabyCrying"], self.condits["StartsNo"])
...
#all this does is create move conditions and store them in a list. The move conditions will be used in a dictionary where they are paired with connections between boats. 
	...
	def __init__(self):
		...
		baby_crying = MoveCondition("Is the baby crying?", "The baby is crying.", "The baby is happy.")
