...
class Boat(object):
	...
	def __init__(self, place, cons, style): #will add name and type later for flavor, but not needed now. 
...
class Map(object):
	...
	def __init__(self):
		...
		for x in range(0, 7):
			...
			self.boats.append(a_boat)
			print "Creating", boat_kinds[x], "at position", x, "connected to", boat_cons[x]
			print "We have created", str(a_boat.kind), "at position", str(a_boat.position), "connected to", a_boat.connections
