...
print("".join(["A", "B"])) #seperated by the character in the parentheses
print("".join(["C", "D"]))
...
both = "AB CD"
print(both[:2])
print(both[-2:]) #print two from the end up to the end
...
for index in range(0, len(string), 8): #moves forward in sets of 8
    bits = string[index: index + 8]
    n = int(bits, 2) #2 is the base since it is binary if 6 it would be hex
    result = result + chr(n)
print(result)
